...
- **右键菜单**：
  ...
  - ⚡️ **功耗曲线和记录**：菜单「工具 → 功耗」里一块概览：当前整机功耗、电从哪来到哪去（插电时「适配器 → 整机 + 充电」，用电池时「电池放电 → 整机」）、平均 / 峰值 / 最低、带时间刻度的曲线，右上角直接切 1 / 6 / 24 小时，切了当场重画；点「功耗曲线…」打开大图。后台一直在记，和喵不喵住无关：每 10 分钟往 `~/Library/Application Support/SleepCat/功耗记录.csv` 写一行（时段、用电量、平均和峰值功耗），曲线窗口右下角能打开这个文件；曲线的采样每分钟落一次盘，重启、更新之后接得上；功耗记录和应用日志都只保留最近 30 天。应用没开或 Mac 睡着的那几段会留白，不会补成直线
