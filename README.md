...
- **右键菜单**：
  ...
  - 🔋 **低电量自动暂停**：用电池时电量低于 20%（可改 10% / 30% / 关闭，或者自定义 5%～90%）持续 1 分钟，就暂停喵住、恢复合盖休眠，避免塞进包里把电耗光；**插上电源会自动恢复**（定时的会接着剩余时间）。插着电源不会暂停，充电器一时带不动、短暂切到电池几秒也不会误停。电量已经很低时手动开启喵住，这一轮不会被暂停。暂停和恢复时会发系统通知（第一次会请求通知权限；关掉了的话，菜单「低电量自动暂停」里有入口打开）
  ...
  - ⚡️ **功耗曲线和记录**：菜单「工具 → 功耗」里一块概览：当前整机功耗、电从哪来到哪去（插电时「适配器 → 整机 + 充电」，用电池时「电池放电 → 整机」）、平均 / 峰值 / 最低、带时间刻度的曲线，右上角直接切 1 / 6 / 24 小时，切了当场重画。后台一直在记，和喵不喵住无关：每 10 分钟往 `~/Library/Application Support/SleepCat/功耗记录.csv` 写一行（时段、用电量、平均和峰值功耗）；曲线的采样每分钟落一次盘，重启、更新之后接得上；功耗记录和应用日志都只保留最近 30 天。应用没开或 Mac 睡着的那几段会留白，不会补成直线
  - ⌨️ **清洁键盘**：暂时禁用所有按键（含亮度音量等功能键），屏幕中央弹出面板，点「清洁完成」恢复。需要「辅助功能」权限
  - 🔒 **合盖也不休眠**：用 `pmset disablesleep` 挡住合盖强制休眠
  - 同时保持屏幕常亮（可选，默认只防系统休眠）
  - 音效：喵（代码合成）/ 呼噜（默认关闭）
- 🏝️ **Duo 岛**：把 MacBook 刘海当灵动岛用（iPhone Duo 风格）——平时隐身，鼠标悬停到刘海或喵住状态变化时流畅展开成黑色胶囊，显示猫猫状态、剩余时间和**实时整机功耗**（展开期间每秒刷新），点按直接切换；菜单可关
- 🌫️ **合盖模糊**：读取 MacBook 内置的铰链角度传感器（HID Sensor 0x20/0x8A），合盖过程中整屏模糊并暗下去，合得越多越糊：100° 起雾、40° 拉满，重新打开反向消散。用系统毛玻璃实时合成，**不读屏幕内容、不需要任何权限**。角度用临界阻尼弹簧平滑，盖子一动按 60 帧跟手，停稳后降到 10 帧省电。盖子停在半路、合到底都保持；只有屏幕正被远程控制 / 屏幕共享 / 录屏持续监看时才自动让开。只画在内建屏幕上，**默认关闭**。调试：`SleepCat --lid-angle` 打印实时角度，`SleepCat --blur-sweep` 模拟一次合盖
<p align="center">
...
- **开机自动启动**：装好后第一次打开默认就开着，菜单「喵住设置 → 开机自动启动」可以关掉（也会出现在「系统设置 › 通用 › 登录项」里）。喵住状态会保留，开机后会自动接着喵
- **调整位置**：按住 ⌘ 拖动菜单栏里的猫
- **连着调设置**：开关和单选那几行点完菜单不会关，可以一次把几个设置都调好
...
- 合盖模糊需要带铰链角度传感器的 MacBook，没有传感器的机型这一项会显示为不可用
