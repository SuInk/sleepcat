...
- **右键菜单**：
  ...
  - ⚡️ **功耗曲线和记录**：菜单里实时显示整机功耗（读 SMC 的 PSTR，插电和用电池都准），子菜单里是近一段时间的曲线，跨度可选 1 / 6 / 24 小时；点「功耗曲线…」打开窗口看细节。**一直在记，和喵不喵住无关**：每 10 分钟往 `~/Library/Application Support/SleepCat/功耗记录.csv` 写一行（时段、用电量、平均和峰值功耗），用表格软件能直接打开；曲线的采样每分钟落一次盘，重启、更新之后接得上。应用没开或 Mac 睡着的那几段会留白，不会补成直线。记录可以关掉
...
- 🌫️ **合盖渐进模糊**：读取 MacBook 内置的铰链角度传感器（HID Sensor 0x20/0x8A），合盖过程中屏幕实时渐进模糊——**铰链边始终清晰，越往远边越糊并渐隐到黑**，模糊半径按高度以 g^1.35 增长、最大 90pt，整体强度按 progress^1.45 增长（取自 iPhone Duo 折叠转场的观感曲线）。100° 起雾、40° 拉满，重新打开反向消散。全部用背景滤镜实时合成，**不读屏幕内容、不需要任何权限**。角度用临界阻尼弹簧平滑，盖子一动按 60 帧跟手，停稳后降到 10 帧省电。盖子停在半路、合到底都保持；只有屏幕正被远程控制 / 屏幕共享 / 录屏持续监看时才自动让开。只画在内建屏幕上。调试：`SleepCat --lid-angle` 打印实时角度，`SleepCat --blur-sweep` 模拟一次合盖
<p align="center">
