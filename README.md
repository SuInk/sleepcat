...
- 🌫️ **Duo 合盖毛玻璃**：读取 MacBook 内置的铰链角度传感器（HID Sensor 0x20/0x8A），合盖过程中屏幕实时结霜——高斯模糊的半径跟着角度连续变化（100° 起雾、40° 拉满），再叠上白釉、斜向高光和磨砂颗粒，复刻 iPhone Duo 折叠时的液态玻璃质感；底下的画面在动，玻璃后面也跟着动。盖子一动就按 60 帧跟手，摊开不动时降到 10 帧省电。重新打开反向消散，盖子停在半路、合到底都保持模糊；只有屏幕正被远程控制 / 屏幕共享 / 录屏持续监看时才自动让开，不会挡住对方看到的画面。只画在内建屏幕上。调试：`SleepCat --lid-angle` 打印实时角度，`SleepCat --blur-sweep` 模拟一次合盖
<p align="center">
